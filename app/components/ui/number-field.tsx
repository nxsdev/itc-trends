...
import { Input, InputRoot, type inputStyles } from "./input"
    ...
    return (
      <NumberFieldRoot className={className} {...props}>
        ...
        >
          <InputRoot
            size={size}
            className="px-0 [&_button]:invalid:border-destructive [&_button]:focus-within:invalid:border-border"
          >
            {prefix}
...
const NumberFieldButton = ({ slot, iconType = "default", ...props }: NumberFieldButtonProps) => {
  ...
  return (
    <Button slot={slot} size="sm" variant="ghost" shape="square" {...props}>
      {icon}
    </Button>
  )
}
