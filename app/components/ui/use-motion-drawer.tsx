...
export const useMotionDrawer = (props: useMotionDrawerProps) => {
  ...
  const _isEntering = animation === "visible"
  ...
  function onRelease(event: React.PointerEvent<HTMLDivElement>) {
    ...
    if (!shouldDrag(event.target) || !swipeAmount || Number.isNaN(swipeAmount)) return
    if (dragStartTime.current === null) return
    // TODO: check for justReleased (to not focus on an element on drag end)
    const timeTaken = dragEndTime.current.getTime() - dragStartTime.current.getTime()
    const distMoved = pointerStart.current - (isVertical(placement) ? event.clientY : event.clientX)
...
export function getTranslate(element: HTMLElement, placement: DrawerPlacement): number | null {
  ...
  if (mat) {
    ...
    return Number.parseFloat(mat[1].split(", ")[isVertical(placement) ? 13 : 12])
  }
  // https://developer.mozilla.org/en-US/docs/Web/CSS/transform-function/matrix
  mat = transform.match(/^matrix\((.+)\)$/)
  return mat ? Number.parseFloat(mat[1].split(", ")[isVertical(placement) ? 5 : 4]) : null
}
